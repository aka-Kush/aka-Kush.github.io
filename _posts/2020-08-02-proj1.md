...
Now, let's start with the assembly of the Hardware:

{% include figure.html image="/images/proj1/IMG_20200802_151821.jpg" alt="Image with just alt text" %}

1. First connect one Jumper wire to the digital pin13 on the Arduino and connect the other end of the jumper wire to one of
   the horizontal holes.

{% include figure.html image="/images/proj1/IMG_20200802_151944.jpg" alt="Image with just alt text" %}

2. Now take the LED and insert the longer leg (Anode) into one of the holes to which the jumper wire is connected and insert the other leg (Cathode)
   to any other group of holes

{% include figure.html image="/images/proj1/IMG_20200802_152027.jpg" alt="Image with just alt text" %}

3. Now take the resistor and connect it's one side to the shorter (cathode) leg of the LED and another side to any other
   group of holes

{% include figure.html image="/images/proj1/IMG_20200802_152119.jpg" alt="Image with just alt text" %}

4. Now take another jumper wire and connect one end to the ground pin on the Arduino and connect the other end of the jumper wire
...
And as the cathode leg of the Arduino and the resistor are in the same of group of holes, they get connected. The other side of the resistor
...
{% include figure.html image="/images/proj1/blink-program.png" alt="Image with just alt text" %}
...
The term 'void' here is used to signify that these functions do not return anything and they don't take any arguments (anything
inside the '()' with a function, is called an Argument and it works as as a value provided to the function whenever we call it).

The function 'pinMode' is used to declare an OUTPUT, which in our case is the digital pin 13.
...
understanding. (Note: an open curly bracket must always be closed after grouping the statements)
...
a Terminator and it used to tell that we have reached the end of the command. Semicolon at the end of a command is mandatory in C/C++ language.
...
the 'board' & 'port' is specified in the tools menu. Also look for any errors in the code itself, especially look for
...
```
{% include video.html id="MK3OeUyHVo" title="Blinking LED example 2.0" %}
...
This was a basic Example; We will keep stepping up. So for many more such projects, stay tuned!
