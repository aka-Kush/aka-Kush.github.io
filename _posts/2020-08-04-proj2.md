...
- Now take the resistors. Connect their one end to the vertical row to which the jumper wire is connected and connect the other leg to the Cathode
...
{% include figure.html image="/images/proj2/Untitled.png" alt="Image with just alt text" %}
...
The function 'pinMode' is used to declare an OUTPUT, which in our case are the digital pins 12,11,10.
...
- First is the initialization statment which basically initializes from where to start the loop. It is executed once at the beginning. This parameter
 is optional.
...
following code given in the curly brackets, everythime the loop is executed.
...
Therefore, first the pin10 turns ON, stays ON for 100 milliseconds and then turn OFF, stays OFF for 100 milliseconds. The same process
